...
class Ga
{
    ...
    public function __construct(ConfigInterface $config, LoggerInterface $logger, \Curl\Curl $curl, Cookie $cookie)
    {
        $this->config = $config;
        $this->logger = $logger;
        $this->curl = $curl;
    ...
    public function init($request)
    {
        if (is_object($request) && $request instanceof Request) {
            ...
            $a = call_user_func_array(array($requestInstance, 'initialize'), $request);
            $this->_init($requestInstance);
        } else {
    ...
    protected function _getParams($uid = '', $documentTitle = '')
    {
        $params = array(
            ...
            'dh' => $this->request->server->get('SERVER_NAME'), // hostname
    ...
    protected function _getLocation()
    {
        ...
        $url .= trim($this->request->server->get('SERVER_NAME'));
        $url .= trim($this->request->server->get('REQUEST_URI'));
    ...
    {
        $this->cookie->set($this->cookieKey, $val, time() + (86400 * 3));
    }
